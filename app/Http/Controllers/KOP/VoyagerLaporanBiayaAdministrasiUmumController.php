...
class VoyagerLaporanBiayaAdministrasiUmumController extends BaseVoyagerBaseController Implements LporanBiayaAdministrasiUmumInterface
{

    public function TotalLaporanBiayaAdministrasiUmum($tahun_pertama, $tahun_kedua, $tahun_ketiga, $nama_biaya)
    {
        return RumusLaporanBiayaAdministrasiUmum::HitungTotalBiayaAdminstrasiUmum($tahun_pertama, $tahun_kedua, $tahun_ketiga, $nama_biaya);
    }

    public function formLaporanBiayaAdministrasiUmumAction(Request $request)
    {
        $company = Company::all();

        return view('vendor.voyager.laporan-biaya-administrasi-umum.forms_laporanbadm', compact('company'));
    }

    public function HitungAkumulasiLPBiayaAdministrasiUmum(Request $r){

        /**
         * Total biaya bag. penjualan
         */
        $total_biaya_lp_adm = $this->TotalLaporanBiayaAdministrasiUmum($r->tahun1, $r->tahun2, $r->tahun3, $r->nama_biaya);
    ...
    // POST BR(E)AD
    public function changedbau(Request $request)
    {

        // dd($request->id);
    if($request->setTo["isConfirmed"] == "true"){
        ...
        $fdata = LaporanBiayaAdministrasiUmum::findOrFail($request->id);
        $dfgdata = [
            'tahun1' => $fdata->tahun1,
            'company_parent_id' => $fdata->company_parent_id,
            'nama_biaya' => $fdata->nama_biaya,
            'tahun2' => $fdata->tahun2,
            'tahun3' => $fdata->tahun3,
            'total_biaya_lp_adm' => $fdata->total_biaya_lp_adm,
            'thn_periode_1' => $fdata->thn_periode_1,
            'thn_periode_2' => $fdata->thn_periode_2,
            'thn_periode_3' => $fdata->thn_periode_3,
        ];


        LaporanBiayaAdministrasiUmum::updateOrCreate(['id' => $fdata->id ], $result_badm);
        ...
        $md = ModulTrackingDataHelpers::ModuleTrackingTransactionData($tb, $dfgdata, $result_badm);
