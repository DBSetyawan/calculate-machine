...
class VoyagerListrikOutputPerjamController extends BaseVoyagerBaseController Implements ListrikOutputInterface
{
    ...
    public function RumusPersenOutputPerjam($output_perjam){

        return RumusListrikOutputPerjam::HitungTotalListrikOutputPerjam($output_perjam);

    }
    public function formListrikOutputPerjam(Request $request)
    {
        $company = Company::all();
        $mesin = Mesin::all();
        $cbagian = KategoriBagian::all();

        return view('vendor.voyager.listrik-output.forms_listrikopjm', compact('company','mesin','cbagian'));
    }

    public function HitungAkumulasiListrikOutputPerjam(Request $r){
            
        /**
         * Hitung Total Listrik Output Perjam
         */
        $HitungTotalListrikOuputPerjam = $this->RumusPersenOutputPerjam($r->output_perjam);
        
        $data_response = [
            'company_parent_id' => $r->company_parent_id,
            'code_lop' => RumusListrikOutputPerjam::generateIDOPERJAM(), 
            'output_perjam' => $r->output_perjam,
