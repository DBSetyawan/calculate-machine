                            ...
                            <div class="panel-body">
                                <div class="contanier">
                                    <div class="form-group">
                                        ...
                                        <input type="text" readonly class="form-control" data-validate-length-range="2" data-validate-words="2" id="code_mesin" value="{{ $dataTypeContent->code_mesin }}" name="code_mesin" placeholder="Nama mesin" required="required" >
                                    </div>
                                    <div class="form-group">
                                        <label for="program_id">Company</label>
                                        <select class="form-control select2" id="company_id" name="company_id" required>
                                            @foreach ($company as $c)
                                        <option value="{{$c->id}}"  @if($dataTypeContent->company_id == $c->id) selected='selected' @endif>{{$c->company_name}}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    <div class="form-group">
                                        ...
                                        <option value="{{$cb->id}}"  @if($dataTypeContent->category_bagian_id == $cb->id) selected='selected' @endif>{{$cb->nama_bagian}}</option>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <select class="form-control select2" id="group_mesin_id" name="group_mesin_id" required>
                                            @foreach ($group_mesin as $cb)
                                        {{-- <option value="{{$c->id}}">{{$c->nama_kategori_mesin}}</option> --}}
                                                <option value="{{$cb->id}}" @if($dataTypeContent->group_mesin_id == $cb->id) selected='selected' @endif>{{$cb->nama_kategori_mesin}}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    <div class="form-group">
                                        ...
                                        <select class="form-control select2" id="asumsi_id" name="asumsi_id" required>
                                            ...
                                            <option value="{{$cb->id}}" @if($dataTypeContent->asumsi_id == $cb->id) selected='selected' @endif>{{$cb->shift}}</option>
                                            @endforeach
                                        </select>   
                                    </div>
                                    ...
                                    {{-- </div> --}}
                                    <div class="form-group">
                                        <label for="url_instagram">Status mesin</label>
                                        <input data-id="{{$dataTypeContent->on_off}}" class="toggle-class" type="checkbox" data-onstyle="success" data-offstyle="danger" data-toggle="toggle" data-on="Aktif" data-off="Non - aktif" {{ $dataTypeContent->on_off ? 'checked' : '' }}>
                                    </div>
                                    <div class="form-group">
                                        <label for="url_instagram">Kapasitas mesin</label>
                                        <input type="text" class="form-control" data-validate-length-range="6" data-validate-words="2" id="capacity_mch" value="{{ $dataTypeContent->capacity_mch }}" name="capacity_mch" placeholder="Jumlah kapasitas mesin">
                                    </div>
                                  
                                <div class="form-group">
                                    <label for="url_instagram">Deskripsi</label>
                                    <input type="text" class="form-control" data-validate-length-range="6" data-validate-words="2" id="deskripsi" value="{{ $dataTypeContent->deskripsi }}" name="deskripsi" placeholder="Deskripsi mesin">
                                </div>
                                <div class="form-group">
                                    <label for="url_instagram">Ampere</label>
                                    <input type="text" class="form-control" data-validate-length-range="6" data-validate-words="2" id="ampere" value="{{ $dataTypeContent->ampere }}" name="ampere" placeholder="Ampere">
                                </div>
                                <div class="form-group">
                                    <label for="url_instagram">Voltase</label>
                                    <input type="text" class="form-control" id="voltase" data-validate-length-range="6" data-validate-words="2" value="{{ $dataTypeContent->voltase }}" name="voltase" placeholder="Voltase">
                                </div>
                                <div class="form-group">
                                    ...
                                    <select class="form-control select2" id="location_mch_id" name="location_mch_id" required>
                                        @foreach ($LocationMachine as $cb)
                                    {{-- <option value="{{$c->id}}">{{ $c->company_name }}</option> --}}
                                            <option value="{{$cb->id}}" @if($dataTypeContent->location_mch_id == $cb->id) selected='selected' @endif>{{$cb->company_name}}</option>
                                        @endforeach
                                    </select>   
                                </div>
                                <div class="form-group">
                                    <label for="url_instagram">Faktor Kali LWBP</label>
                                    <input type="text" class="form-control" data-validate-length-range="6" readonly value="{{ $dataTypeContent->faktor_kali_lwbp}}" data-validate-words="2" id="faktor_kali_lwbp" name="faktor_kali_lwbp" placeholder="Faktor kali LWBP">
                                </div>
                                <div class="form-group">
                                    <label for="url_instagram">Faktor Kali WBP</label>
                                    <input type="text" class="form-control" id="faktor_kali_wbp" readonly data-validate-length-range="6" value="{{ $dataTypeContent->faktor_kali_wbp }}" data-validate-words="2" name="faktor_kali_wbp" placeholder="Faktor kali WBP">
                                </div>
                            </div>
                            {{-- <button type="submit" class="btn btn-primary pull-right save_mach">Simpan</button>&nbsp; --}}
                            <button type="button" class="btn btn-primary pull-right save_mach">Simpan</button>&nbsp;
...
@section('javascript')
    <script>
        let machineID = "{{ $dataTypeContent->id }}";
        var params = {};
        var $file;
        ...
        $(function() {
            $('.toggle-class').change(function() {
                ...
                var user_id = machineID; 
                
                $.ajax({
                    type: "POST",
                    dataType: "json",
                    url: "{{ route('mesin.stats.change') }}",
                    data: {'status': status, 'machineID': user_id},
    ...
    $(document).ready(function() {
        ...
        $('.save_mach').click(function(event) {
            ...
            var formData = {
                ...
                'capacity_mch'        : $('input[name=capacity_mch]').val(),
            ...
            Swal.fire({
                ...
                confirmButtonText: `ya, simpan mesin terbaru ✓`,
                ...
                $.ajax({
                    type        : 'POST',
                    url         : "{{ route('mesin.updateMasterMachine.master') }}",
                    data        : formData, 
                    dataType    : 'json', 
                    encode          : true
                })
                ...
                //     $("#total_biaya_perbulan").val("Rp "+formatCurrency(Math.round(data.total_biaya_perbulan)));
                    ...
                    if(data.sts == 'true'){
                        ...
                        pesanStore.fire({
                                icon: 'success',
                                title: 'Data berhasil disimpan, perubahan telah direcord'
                            })
                        ...
                        pesanStore.fire({
                                icon: 'info',
                                title: 'Data berhasil disimpan, tidak ada perubahan apapun'
                            })
